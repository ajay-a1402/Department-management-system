...
module.exports.createUserList  = async (req, res) => {
    ...
    const unique = [...new Map(body.map(item =>
        [item[key], item])).values()];

        unique.map(async data=>{
        if(isNull(data.employeeId)){
            return ReE(res, {message:'please Enter Employee ID'}, HttpStatus.BAD_REQUEST)
        }
        ...
        [err, list] = await to(User.find({employeeId:data.employeeId,enabled:true,email:data.email}));
        if(err){
      ...
      return ReS(res, { message: "User Create Succefully",newList }, HttpStatus.OK) 
  } 